function renderHand() {
  ...
  function buildCard(card, idx) {
    ...
    const _powColor = (card.ability && typeof ABILITY_ICONS !== 'undefined' && ABILITY_ICONS[card.ability])
      ? ABILITY_ICONS[card.ability].color : '#b877ff';

    const div   = document.createElement('div');

    div.className = 'hand-card' + (isSel ? ' selected' : '') + (card.ability ? ' special-power' : '');
    if (card.ability) div.style.setProperty('--pow', _powColor);
    ...
    div.innerHTML =
      ...
      (card.ability ? '<div style="position:absolute;bottom:34px;left:50%;transform:translateX(-50%);z-index:6;width:30px;height:30px;border-radius:6px;background:rgba(8,6,20,0.34);border:1px solid '+_powColor+'88;box-shadow:0 0 10px '+_powColor+'55;display:flex;align-items:center;justify-content:center;pointer-events:none;animation:abilityPulse 2.5s ease-in-out infinite;" title="'+ab(card.ability)+'">'+_getAbilitySvg(card.ability)+'</div>' : '') +
    ...
    div.onmouseleave = function() { hideTip(); };
      ...
      div.addEventListener('touchstart', function(e) {
        ...
        let _longPressTimer = setTimeout(() => {
          ...
          if (panel && !panel.classList.contains('open')) {
            setTimeout(() => { panel.classList.add('open'); if (tab) { tab.textContent = '▼'; } }, 10);
          }
        }, 480);
