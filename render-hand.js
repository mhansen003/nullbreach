function renderHand() {
  ...
  function buildCard(card, idx) {
    ...
    div.innerHTML =
      ...
      (card.ability ? '<div style="position:absolute;bottom:22px;left:50%;transform:translateX(-50%);z-index:6;width:22px;height:22px;border-radius:4px;background:rgba(10,8,24,0.62);display:flex;align-items:center;justify-content:center;pointer-events:none;animation:abilityPulse 2.5s ease-in-out infinite;" title="'+ab(card.ability)+'">'+_getAbilitySvg(card.ability)+'</div>' : '') +
